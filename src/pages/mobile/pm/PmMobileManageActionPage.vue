<template>
  <q-page class="mobile-page mobile-page--with-primary-action">
    <q-form id="pm-mobile-action-form" class="mobile-page-stack" @submit="onSubmit">
      ...
      <MobileCard
        v-if="action === 'transaction'"
        title="Transaction"
        subtitle="Quick PM-side financial entry."
      >
        <div class="mobile-form-stack">
          <q-select
            v-model="form.transac_type"
          ...
          />
          <q-select
            v-model="form.transac_to"
            :options="toRoleOptions"
            label="To"
            outlined
            dense
            :disable="!form.transac_from"
            :rules="[required, differentTransactionSide]"
          />
      ...
      <MobileCard
        v-else-if="action === 'reminder'"
        title="Reminder"
        subtitle="Create a dated follow-up."
      >
        <div class="mobile-form-stack">
          <q-select
      ...
      <MobileCard
        v-else-if="action === 'task'"
        title="Task"
        subtitle="Create a property task using the same mxrecords structure as web."
      >
        <div class="mobile-form-stack">
      ...
      <MobileCard
        v-else-if="action === 'document'"
        title="Document"
        subtitle="Upload a property file or photo."
      >
        <div class="mobile-form-stack">
          <q-file
      ...
      <MobileCard
        v-else-if="action === 'service'"
        title="Service"
        subtitle="Create a service record for the selected property."
      >
        <div class="mobile-form-stack">
          <q-select
      ...
      <MobileCard
        v-else-if="action === 'asset'"
        title="Asset"
        subtitle="Add core asset information onsite."
      >
        <div class="mobile-form-stack">
          <q-input
      ...
      <MobileCard
        v-else-if="action === 'inventory'"
        title="Inventory is lease-bound"
        subtitle="Open inventory from a property card so the active lease context is known."
      >
        <button
          type="button"
          class="mobile-list-row mobile-list-row--button"
          @click="pushMobile('/mobile/pm/property')"
        >
          ...
          <div class="mobile-list-row__body">
            ...
            <div class="mobile-list-row__meta">
              Choose a property, then open its current lease inventory.
            </div>
          </div>
        </button>
      </MobileCard>
      ...
      <MobileCard
        v-else
        :title="`${config.title} mobile form`"
        subtitle="This manage action is not configured yet."
      />
    </q-form>
...
const previewProperties = [
  {
    id: 'preview-property-main',
    property_id: 'preview-property-main',
...
const differentTransactionSide = (val) =>
  val !== form.transac_from || 'From and To cannot be the same'
...
const saveDocument = async (propertyId) => {
  ...
  const recordId = await createMobileDocument(`properties/${propertyId}/documents`, {
    ...
    size: file.size || null,
    uploaded_by_role:
      userDataStore.getUserRoleForProperty(propertyId)?.role || userDataStore.accountType || null,
    upload_date: now,
...
const saveTask = async (propertyId) => {
  ...
  const userName =
    userDataStore.user?.displayName || userDataStore.user?.email || 'Property Manager'
...
const saveService = async (propertyId) => {
  const now = new Date().toISOString()
  const propertyLabel =
    propertyOptions.value.find((property) => property.value === propertyId)?.label || propertyId
