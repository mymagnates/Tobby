...
const fallbackTaskItems = computed(() =>
  pmFeedItems.map((item, index) => ({
    ...
    kind: 'task',
    to: item.to || '/mobile/pm/manage/view/tasks',
    eventDate: '',
  })),
)
...
const quickStats = computed(() => [
  ...
  { label: 'Records', count: allItems.value.length, value: 'records' },
])

const allItems = computed(() => {
  const realItems = [...taskItems.value, ...reminderItems.value]
  const source =
...
const filteredItems = computed(() => {
  ...
  if (activeFilter.value === 'records') return allItems.value
...
const sectionTitle = computed(() => {
  ...
  if (activeFilter.value === 'records') return 'Recent records'
