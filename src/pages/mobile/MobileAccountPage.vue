<template>
  <q-page class="mobile-page mobile-page--with-primary-action">
    <q-form id="mobile-account-form" class="mobile-page-stack" @submit="saveProfile">
      ...
      <MobileCard
        v-if="showQuotaTracking"
        title="Usage & Credits"
        :subtitle="quotaSubtitle"
        :status="quotaStatus"
        :tone="quotaStatusTone"
      >
        <div v-if="billingLoading" class="mobile-quota-stack">
          <div v-for="index in 3" :key="index" class="mobile-quota-row">
            <div class="mobile-quota-row__header">
              <q-skeleton type="text" width="34%" />
              <q-skeleton type="text" width="22%" />
            </div>
            <q-skeleton type="rect" height="8px" class="mobile-quota-row__skeleton" />
          </div>
        </div>

        <div v-else class="mobile-quota-stack">
          <div class="mobile-quota-balance">
            <div>
              <div class="mobile-quota-balance__label">Credit Balance</div>
              <div class="mobile-quota-balance__meta">Read-only in mobile app</div>
            </div>
            <div class="mobile-quota-balance__value">{{ creditBalanceDisplay }}</div>
          </div>

          <div v-for="item in quotaRows" :key="item.key" class="mobile-quota-row">
            <div class="mobile-quota-row__header">
              <span class="mobile-quota-row__label">{{ item.label }}</span>
              <span class="mobile-quota-row__value">{{ item.value }}</span>
            </div>
            <q-linear-progress
              rounded
              size="8px"
              ...
              track-color="grey-3"
            />
            <div class="mobile-quota-row__footer">{{ item.note }}</div>
          </div>

          <div v-if="billingError" class="mobile-quota-note">
            Latest usage is unavailable. Showing fallback values.
          </div>
        </div>
      </MobileCard>
...
const showQuotaTracking = computed(() => normalizedRole.value === 'pm')
...
const clampRatio = (used, limit) => {
  const safeLimit = Number(limit)
  if (!Number.isFinite(safeLimit) || safeLimit <= 0) return 0
  return Math.min(1, Math.max(0, Number(used || 0) / safeLimit))
}

const usageColor = (ratio) => {
...
const formatGb = (mb) => (Math.max(0, Number(mb || 0)) / 1024).toFixed(1)

const planName = computed(
  () => billingSummary.value?.plan_name || billingSummary.value?.plan || 'Free',
)
const subscriptionStatus = computed(() =>
  String(billingSummary.value?.subscription_status || billingSummary.value?.status || 'inactive'),
)
const gateStatus = computed(() => String(billingSummary.value?.gate_status || 'ok').toLowerCase())
const quotaStatus = computed(() => {
  if (billingLoading.value) return 'Loading'
  if (billingError.value) return 'Offline'
  if (gateStatus.value && gateStatus.value !== 'ok') return 'Needs attention'
  return planName.value
})
const quotaStatusTone = computed(() => {
  if (billingError.value) return 'warning'
  if (gateStatus.value && gateStatus.value !== 'ok') return 'danger'
  return 'neutral'
})
const quotaSubtitle = computed(() => {
  const status = subscriptionStatus.value.replace(/_/g, ' ')
  return `${planName.value} plan · ${status}`
})

const creditBalance = computed(() =>
  Number(
    billingCredits.value?.balance ??
      billingCredits.value?.credits ??
      billingCredits.value?.credit_balance ??
      0,
  ),
)
const creditBalanceDisplay = computed(() => formatCount(creditBalance.value))

const quotaRows = computed(() => {
  const aiUsed = pickUsageNumber(['ai_tokens_used', 'ai_token_used', 'tokens_used', 'token_used'])
  const aiLimit = pickUsageNumber([
    'ai_tokens_limit',
    'ai_token_limit',
    'tokens_limit',
    'token_limit',
  ])
  const storageUsed = pickUsageNumber(['storage_used_mb', 'storage_mb_used'])
  const storageLimit = pickUsageNumber(['storage_limit_mb', 'storage_mb_limit'])
  const propertiesUsed = pickUsageNumber(['properties_used', 'property_used'])
  const propertiesLimit = pickUsageNumber(['properties_limit', 'property_limit'])
  ...
  const rows = [
    {
      ...
      used: aiUsed,
      limit: aiLimit,
      value: `${formatCount(aiUsed)} / ${formatCount(aiLimit)}`,
      note: `${formatCount(Math.max(0, aiLimit - aiUsed))} left this month`,
    },
    {
      key: 'storage',
      label: 'Storage',
      used: storageUsed,
      limit: storageLimit,
      value: `${formatGb(storageUsed)} / ${formatGb(storageLimit)} GB`,
      note: `${Math.round(clampRatio(storageUsed, storageLimit) * 100)}% used`,
    },
    {
      key: 'properties',
      label: 'Properties',
      ...
      value: `${formatCount(propertiesUsed)} / ${formatCount(propertiesLimit)}`,
      note: `${formatCount(Math.max(0, propertiesLimit - propertiesUsed))} available`,
    },
  ]

  return rows.map((row) => {
    const ratio = clampRatio(row.used, row.limit)
    return {
      ...row,
      ratio,
...
const loadBillingData = async () => {
  if (!showQuotaTracking.value) return
  billingLoading.value = true
  billingError.value = false
  try {
