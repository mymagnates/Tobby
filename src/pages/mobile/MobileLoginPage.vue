...
import {
  getMobileHomePathForStore,
  isNativeMobileRuntime,
  isPmMobilePath,
} from 'src/utils/mobileRuntime'
...
const submit = async () => {
  ...
  try {
    ...
    const target =
      redirect.startsWith('/mobile') && (!isNativeMobileRuntime() || isPmMobilePath(redirect))
        ? redirect
        : getMobileHomePathForStore(userDataStore)
